...
const EditorPanel = ({ selectedElement, onUpdate, onClose }: EditorPanelProps) => {
    ...
    const handleChange = (field: string, value: string) => {
        const newValues: any = { ...values, [field]: value };
        setValues(newValues);
        onUpdate({ [field]: value });
    };

    const handleStyleChange = (styleName: string, value: string) => {
        const newStyles = { ...(values as any).styles, [styleName]: value };
        setValues({ ...(values as any), styles: newStyles });
        ...
        <div className='absolute top-4 right-4 w-80 bg-white rounded-lg shadow-xl border border-gray-200 p-4 z-50 animate-in fade-in slide-in-from-right-5'>
            ...
            <div className='space-y-4 text-black'>
                <div>
                    ...
                    <textarea value={values.text} onChange={(e) => handleChange('text', e.target.value)} className='w-full text-sm p-2 border rounded-md focus:ring-2 focus:ring-blue-500 outline-none min-h-20' />
                </div>
                <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">Class Name</label>
                    <input type='text' value={values.className || ''} onChange={(e) => handleChange('className', e.target.value)} className='w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-blue-500 outline-none' />
                </div>
                <div className='grid grid-cols-2 gap-3'>
                    <div>
                        ...
                        <input type='text'
                            ...
                            className='w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-blue-500 outline-none' />
                    </div>
                    <div>
