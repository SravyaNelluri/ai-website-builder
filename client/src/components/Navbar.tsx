...
console.log('Credits response:', data) // Add this line
...
getCredits()
  ...
  return (
    <>
   <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 bg-black backdrop-blur border-b text-white border-slate-800">
       <a href="https://prebuiltui.com">
       <Link to='/'>
             <img src={assets.logo} alt="log" className='h-5 sm:h-7' />
             </Link>
          </a>
       ...
       {/* Mobile Menu */}
        {menuOpen && (
          <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
            ...
            <Link to='/' onClick={() => setMenuOpen(false)}>My Projects</Link>
