 ...
 useImperativeHandle(ref, ()=> ({
    getCode: ()=>{
       ...
       doc.querySelectorAll('.ai-selected-element,[data-ai-selected]').forEach((el)=>{
         ...
         // 2. Return injectedstyle +script from the document
   ...
   useEffect(() =>{
    const handlMeessage =(event: MessageEvent)=>{
          if(event.data && event.data.type === 'ELEMENT_SELECTED'){
             // Normalize payload to match EditorPanel expectations
          ...
          }else if(event.data && event.data.type === 'CLEAR_SELECTION'){
             setSelectedElement(null)
          }
    }
    window.addEventListener('message',handlMeessage);
    return ()=> window.removeEventListener('message', handlMeessage);
   },[])
