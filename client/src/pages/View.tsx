...
const View = () => {
  ...
  const fetchProject = async () => {
    try {
      ...
      if (!data.project.current_code || data.project.current_code.trim() === '') {
  ...
  const downloadCode = () => {
    ...
    if (!code) {
      if (isGenerating) {
        toast.error('Please wait for the code generation to complete')
        return
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.project.current_code && data.project.current_code.trim() !== '' && hasNewCode) {
          previousCodeLength = data.project.current_code.length
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center gap-2 sm:min-w-90 text-nowrap'>
          ...
          <div className='max-w-64 sm:max-w-xs'>
            ...
            <p className='text-xs text-gray-400 -mt-0.5'>Viewing project</p>
          </div>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm '>
         <button onClick={saveProject} disabled={isSaving} className='max-sm:hidden bg-gray-800 hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 rounded-sm transistion-colors border border-gray-700'>
         ...
         <button onClick={downloadCode} className='bg-gradient-to-br from-blue-700 to-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transistion-colors'>
          <ArrowBigDownDashIcon size={16} />
          Download
         </button>
         <button onClick={togglePublish} className='bg-gradient-to-br from-blue-700 to-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transistion-colors' >
          {project.isPublished ? 
      ...
      <div className='flex-1 flex overflow-hidden'>
       ...
       <Sidebar isMenuOpen={isMenuOpen} project={project} setProject={(p) => setProject(p)} isGenerating={isGenerating} setIsGenerating={setIsGenerating}/>
        ...
        <div className='flex-1 p-2 pl-0 h-full'>
          {isGenerating && (!project.current_code || project.current_code.trim() === '') ? (
            <LoaderSteps />
          ) : (
